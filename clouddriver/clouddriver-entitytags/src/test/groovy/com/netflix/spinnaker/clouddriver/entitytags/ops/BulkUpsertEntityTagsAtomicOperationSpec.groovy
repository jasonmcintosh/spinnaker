...
class BulkUpsertEntityTagsAtomicOperationSpec extends Specification {
  ...
  def entityTagsProvider = Mock(EntityTagsProvider)
  ...
  void "should perform bulk operation"() {
    ...
    // one get() per tag id since we are not aware of any existing tags
    1000 * entityTagsProvider.get(_) >> Optional.empty()
    // partitioned into 20 batches of 50
  ...
  void 'should merge tags with duplicate entityRefIds'() {
    ...
    // 2 remaining ids after merge, each looked up
    2 * entityTagsProvider.get(_) >> Optional.empty()
  ...
  void 'should create new tag if none exists'() {
    ...
    tag.lastModifiedBy == "anonymous"
    tag.lastModified != null

    1 * accountCredentialsProvider.getAll() >> { return [testCredentials] }
    1 * entityTagsProvider.get("aws:servergroup:orca-v001:100:us-east-1") >> Optional.empty()
  ...
  void 'should not halt on exception, but include in results'() {
    ...
    4 * accountCredentialsProvider.getAll() >> { return [testCredentials] }
    3 * entityTagsProvider.get(_) >> Optional.empty()
    1 * entityTagsProvider.bulkIndex(_)
  }
